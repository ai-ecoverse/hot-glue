...
**And then the package became the toolchain.** The tarball used to carry stage 0: `npx @ai-ecoverse/hot-glue` ran the TypeScript expander, and a caller who wanted a binary either installed a hundred megabytes of optimizer or vendored `as.wasm` out of a checkout by hand — which is what a downstream consumer was in fact reduced to, pinning the artifact by commit and the package by npm with nothing anywhere to make the two agree. They agree now. `prepack` runs the bootstrap, the three organs ship beside the `.hma` sources that determine them, and `dist/drive.js` is the playground's driver with a real filesystem behind `path_open`: fifty lines of WASI, no wasmtime, no `node:wasi`, and no expander of its own — so `hotglue -w` assembles with nothing installed but Node. The gate proves the artifacts instead of trusting them: it expands `as.hma` with the shipped toolchain, assembles it with the shipped assembler, and requires the bytes that come out to be the shipped assembler — stage 3 again, run from the outside, against what a stranger downloaded. Stage 0 stays in the repository as what it always was: the thing that makes the first binary, and nothing after it. One asymmetry fell out along the way, of the kind that costs somebody an afternoon: a `(use as.hma)` off stdin had always found the shipped source while a bare `hotglue as.hma` had not, the entry file being the one name read as a literal path. The lookup path answers both now.
